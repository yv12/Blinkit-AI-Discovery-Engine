...
- Merging in ~4,098 Mouthshut units (4.4% of the merged 93,393-unit corpus) shifted a few
  communities across the `min_community_size=3` boundary: **5 themes + 851 emerging signals**
  post-merge vs. 40/819 pre-merge - the qualifying-theme count held steady, only the long tail
  moved slightly.
- Mouthshut units are **not siloed**: they landed in all 5 themes, confirmed by
...
(156,219 Google Play + 1,000 Mouthshut) → 93,393 units (89,295 + 4,098) → 5 themes + 851
emerging signals → same 4/8 questions `"sufficient"` (Q1/Q3/Q4/Q6) → validation modularity
0.8366, 4/5 cross-segment stable. `app.py` smoke-tested against the regenerated artifacts
