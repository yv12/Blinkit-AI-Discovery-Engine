...
| S8-02 | Theme maps to no question | Off-topic/noise cluster | Nothing dropped silently | **Decided:** bucket under `insights.json`'s `"uncategorized"` key (16/40 themes, 780/819 signals on the real corpus); still fully present in `themes.json`, just not force-mapped | Cosmetic |
...
| S9-03 | Coherence undefined for singletons | Community size 1 | No divide-by-zero | `_coherence()` skips (returns `null` fields, not a crash) any theme with < 2 members with embeddings; doesn't occur among the real 40 qualifying themes (all size >= 40) but guarded defensively | Cosmetic |
